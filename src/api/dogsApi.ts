...
const DOG_API_ROUTE = 'https://dog.ceo/api';
const ALL_DOGS_ROUTE = `${DOG_API_ROUTE}/breeds/list/all`;
...
export const getDogImageApi = async (dog: string): Promise<string> => {
	...
	const result = await axios.get(
		`${DOG_API_ROUTE}/breed/${dogRoute}/images/random`
	);

	if (result.status !== 200 || result.data.status !== 'success') {
		throw new Error('Api image call status was not a success');
	}
